        ...
        return(
            <div style={{marginTop:'56px', overflowX: 'hidden'}} className="container-fluid">
                {this.state.playing.map((playing, index) => {
                    ...
                    if(index === random)
                        return (<div style={{textAlign:'start', backgroundPosition:'center',backgroundImage:'url(https://image.tmdb.org/t/p/original/'+playing.backdrop_path+')',  maxHeight:'600px',minHeight:'500px', backgroundSize:'cover', loading:'lazy'}} class="p-4 p-md-5 mb-4 text-white rounded bg-dark">
                        <div class="col-md-6 px-0 bg-text">
                        <h1 style={{fontSize:'xx-large', color:'white', textAlign:'start', padding:'10px'}} class="display-4 fst-italic">{playing.original_title}</h1>
                        <p style={{fontSize:'inherit', color:'white', textAlign:'start', padding:'10px'}} class="lead my-3">{short}</p>
                        <p style={{textAlign:'start', padding:'10px'}} class="lead mb-0"><a href={"/movie/"+playing.id} class="btn btn-primary text-white fw-bold">Details</a></p>
                        </div></div>)
                })}
